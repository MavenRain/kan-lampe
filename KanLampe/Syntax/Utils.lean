...
import KanLampe.Syntax.Rules
import KanLampe.Tp

set_option autoImplicit true

/-!
# KanLampe.Syntax.Utils

Port of `Lampe.Syntax.Utils`.  Provides the `MonadUtil` class and the
`make*` helper functions used by the Noir DSL frontend to translate
surface syntax into core `KanLampe` terms.
-/
...
| `(ident|Field) => ``(KanLampe.Kind.field)
| `(ident|u1) => ``(KanLampe.Kind.u 1)
| `(ident|u8) => ``(KanLampe.Kind.u 8)
| `(ident|u16) => ``(KanLampe.Kind.u 16)
| `(ident|u32) => ``(KanLampe.Kind.u 32)
| `(ident|u64) => ``(KanLampe.Kind.u 64)
| `(ident|u128) => ``(KanLampe.Kind.u 128)
| k => throwError "Invalid generic kind {k}"

/-- Turns the provided `kind` back into its syntactic term representation. -/
def quoteKind [MonadUtil m] (kind : KanLampe.Kind) : m (TSyntax `term) :=
  match kind with
  | .u 1 => ``(KanLampe.Kind.u 1)
  | .u 8 => ``(KanLampe.Kind.u 8)
  | .u 16 => ``(KanLampe.Kind.u 16)
  | .u 32 => ``(KanLampe.Kind.u 32)
  | .u 64 => ``(KanLampe.Kind.u 64)
  | .u 128 => ``(KanLampe.Kind.u 128)
  | .field => ``(KanLampe.Kind.field)
  | .type => ``(KanLampe.Kind.type)
  | _ => throwUnsupportedSyntax

/-- Builds a literal kind from the provided syntax tree, throwing an error if invalid. -/
partial def makeKindValue [MonadUtil m] : (kind: TSyntax `noir_kind) → m KanLampe.Kind
| `(noir_kind|Field) => pure KanLampe.Kind.field
| `(noir_kind|u1) => pure $ KanLampe.Kind.u 1
| `(noir_kind|u8) => pure $ KanLampe.Kind.u 8
| `(noir_kind|u16) => pure $ KanLampe.Kind.u 16
| `(noir_kind|u32) => pure $ KanLampe.Kind.u 32
| `(noir_kind|u64) => pure $ KanLampe.Kind.u 64
| `(noir_kind|u128) => pure $ KanLampe.Kind.u 128
| `(noir_kind|Type) => pure $ KanLampe.Kind.type
...
| `(noir_kind|Field) => ``(KanLampe.Kind.field)
| `(noir_kind|u1) => ``(KanLampe.Kind.u 1)
| `(noir_kind|u8) => ``(KanLampe.Kind.u 8)
| `(noir_kind|u16) => ``(KanLampe.Kind.u 16)
| `(noir_kind|u32) => ``(KanLampe.Kind.u 32)
| `(noir_kind|u64) => ``(KanLampe.Kind.u 64)
| `(noir_kind|u128) => ``(KanLampe.Kind.u 128)
| `(noir_kind|Type) => ``(KanLampe.Kind.type)
| k => throwError "Unsupported kind {k}"
...
| `(noir_gen_val|$t:noir_type) => do
  pure ⟨←makeNoirType t, KanLampe.Kind.type⟩
...
| `(noir_type|u1) => ``(KanLampe.Tp.u 1)
| `(noir_type|u8) => ``(KanLampe.Tp.u 8)
| `(noir_type|u16) => ``(KanLampe.Tp.u 16)
| `(noir_type|u32) => ``(KanLampe.Tp.u 32)
| `(noir_type|u64) => ``(KanLampe.Tp.u 64)
| `(noir_type|u128) => ``(KanLampe.Tp.u 128)
| `(noir_type|i1) => ``(KanLampe.Tp.i 1)
| `(noir_type|i8) => ``(KanLampe.Tp.i 8)
| `(noir_type|i16) => ``(KanLampe.Tp.i 16)
| `(noir_type|i32) => ``(KanLampe.Tp.i 32)
| `(noir_type|i64) => ``(KanLampe.Tp.i 64)
| `(noir_type|i128) => ``(KanLampe.Tp.i 128)
| `(noir_type|bool) => ``(KanLampe.Tp.bool)
| `(noir_type|Field) => ``(KanLampe.Tp.field)
...
| `(noir_type|FmtString<$n:noir_gen_val, $tp:noir_gen_val>) => do
  ...
  ``(KanLampe.Tp.fmtStr $(n_gen.value) $tp)
| `(noir_type|Unit) => `(KanLampe.Tp.unit)
| `(noir_type|$i:ident) => ``($i)
| `(noir_type|&$tp) => do ``(KanLampe.Tp.ref $(←makeNoirType tp))
| `(noir_type|$structName:noir_ident < $generics,* >) => do
  ...
  | "Array" => do
      ...
      ``(KanLampe.Tp.array $(elemGeneric.value) $(sizeGeneric.value))
  | "Vector" => do
    if gensLen != 1 then
    ...
    ``(KanLampe.Tp.vector $(elemGeneric.value))
  | "Tuple" => do
    let generics := (←makeGenericVals genericsList).map fun g => g.value
    ``(KanLampe.Tp.tuple none $(←makeListLit generics))
  | _ => do
    let structIdent := makeStructDefIdent (←makeNoirIdent structName)
    let generics := (←makeGenericVals genericsList).map fun g => g.value
    ``(KanLampe.Struct.tp $structIdent $(←makeHListLit generics))
...
| `(noir_type|λ( $param_types,* ) → $ret:noir_type) => do
  ...
  ``(KanLampe.Tp.fn $paramTypes $returnType)
...
| 0 => ``(KanLampe.Builtin.Member.head)
| n + 1 => do ``(KanLampe.Builtin.Member.tail $(←makeMember n))
