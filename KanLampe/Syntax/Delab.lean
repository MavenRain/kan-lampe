...
import KanLampe.Syntax.Elab

set_option autoImplicit true

/-!
# KanLampe.Syntax.Delab

Port of `Lampe.Syntax.Delab`.  Provides the delaborators that
pretty-print `KanLampe.Expr` and `KanLampe.STHoare` using the Noir
DSL surface syntax, gated on the `KanLampe.pp.Expr` and
`KanLampe.pp.STHoare` options.
-/

open Lean PrettyPrinter Delaborator SubExpr

register_option KanLampe.pp.Expr : Bool := {
  defValue := false
  descr := "Pretty print KanLampe.Expr using the KanLampe syntax"
}

register_option KanLampe.pp.STHoare : Bool := {
  defValue := false
  descr := "Pretty print KanLampe.STHoare using the KanLampe syntax"
}

namespace KanLampe

/-- Switch whether the `Expr` delaborator fires based off the `KanLampe.pp.Expr` option -/
abbrev whenDelabExprOption : DelabM α → DelabM α := whenDelabOptionSet `KanLampe.pp.Expr
...
/-- Because of the choice to index KanLampe.Expr terms with `Prop` types rather than `Bool`s we need to
...
/-- Extract the inner KanLampe expression from a block or enclosing `⸨⋯⸩` block -/
...
@[app_delab KanLampe.Expr.readRef]
    ...
    let letBinding ←
      if val.isAppOf ``KanLampe.Expr.ref then
        ...
        whenFullyApplied val do
          let val := val.getArg! 3
          `(noir_expr|let $(⟨var⟩) = $(extractInnerLampeExpr (← delab val)))
      else if val.isAppOf ``KanLampe.Expr.loop then
        whenFullyApplied val do
          `(noir_expr|$(extractInnerLampeExpr (← delab val)))
      else if val.isAppOf ``KanLampe.Expr.ite then
        whenFullyApplied val do
        if bindingType.isAppOf ``KanLampe.Tp.unit then
          `(noir_expr|$(extractInnerLampeExpr (← delab val)))
        else
          `(noir_expr|let $(⟨var⟩) = $(extractInnerLampeExpr (← delab val)))
      else
...
@[app_delab KanLampe.FuncRef.lambda]
...
@[app_delab KanLampe.FuncRef.decl]
...
@[app_delab KanLampe.FuncRef.trait]
...
def delabLam : Delab := whenDelabExprOption getExpr >>= fun expr =>
  whenFullyApplied expr do
    ...
    let some (_ , argTps) := argTps.listLit?
      | throwError "expected list lit for arg types in KanLampe.Expr.lam, got {← ppExpr argTps}"
...
@[app_delab KanLampe.Expr.litNum]
...
@[app_delab KanLampe.Expr.callBuiltin]
...
def delabLoop : Delab := whenDelabExprOption getExpr >>= fun expr =>
  whenFullyApplied expr do
    ...
    let (var, body) ← match (← delab <| expr.getArg! 5) with
      | `(fun $var => $body) =>
        pure (var, body)
      | _ => throwError "Malformed KanLampe.Expr.loop, body is not a lambda, got {← ppExpr <| expr.getArg! 5}"
...
@[app_delab KanLampe.Expr.constFp]
...
@[app_delab KanLampe.Expr.constU]
...
@[app_delab KanLampe.Expr.litStr]
...
/-- Switch whether the `STHoare` delaborator fires based off the `KanLampe.pp.STHoare` option -/
abbrev whenDelabSTHoareOption : DelabM α → DelabM α := whenDelabOptionSet `KanLampe.pp.STHoare

@[app_delab KanLampe.STHoare]
