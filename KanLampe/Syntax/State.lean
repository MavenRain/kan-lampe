...
/-!
# KanLampe.Syntax.State

Port of `Lampe.Syntax.State`.  Defines the `Binder`, `LambdaParam`,
`DSLState`, `MonadDSL`, `Args`, and `LValueRef` types used by the
Noir DSL frontend.
-/
...
/-- The state for the desugaring process from the DSL syntax into native Lean/KanLampe constructs. -/
...
def isAutoDerefd [MonadDSL m] (i : Name) : m Bool := do
  let st ← get
