...
import KanLampe.Syntax.Builders
import KanLampe.Syntax.Rules
import KanLampe.Syntax.State
import KanLampe.Syntax.Utils

/-!
# KanLampe.Syntax.Elab

Port of `Lampe.Syntax.Elab`.  Provides the `elab`/`macro` entrypoints
for the Noir DSL frontend: `noir_def`, `noir_trait_impl`,
`noir_global_def`, `noir_type_alias`, `noir_struct_def`,
`noir_trait_def`, plus the debugging quotations `expr!![`, `type!![`,
`kind!![`, `gVal!![`.
-/
