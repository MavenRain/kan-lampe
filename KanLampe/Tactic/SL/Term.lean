...
/-!
# KanLampe.Tactic.SL.Term

Port of `Lampe.Tactic.SL.Term`.  Defines `SLTerm`, the reified shape
of a separation-logic formula used by the `sl` tactic layer, together
with a collection of internal bookkeeping lemmas (`do_star_assoc`,
`congr_star_r`, ...) and the `linearize` / `sort` / `norm` /
`surfaceExis` / `split_by` normalization helpers.
-/
...
partial def solvesSingleton (lhs : SLTerm) (rhsV : Lean.Expr) : TacticM Bool :=
  ...
  | SLTerm.exi _ (Lean.Expr.lam _ _ body _) => do
      solvesSingleton (← parseSLExpr body) rhsV
  | _ => pure false

namespace Internal

theorem do_star_assoc {p} (a b c : SLP (State p)) :
    ((a ⋆ b) ⋆ c) = (a ⋆ (b ⋆ c)) := by
  kan_rw [SLP.star_assoc]

theorem congr_star_r {p} (a b c : SLP (State p)) (eq : b = c) :
    (a ⋆ b) = (a ⋆ c) := by
  kan_cases eq
  kan_rfl

theorem do_star_comm {p} (a b : SLP (State p)) : (a ⋆ b) = (b ⋆ a) := by
  kan_rw [SLP.star_comm]

theorem do_star_swap {p} (a b c : SLP (State p)) :
    (a ⋆ b ⋆ c) = (b ⋆ a ⋆ c) := by
  kan_rw [<- SLP.star_assoc]
  kan_rw [@SLP.star_comm (State p) _ a b]
  kan_rw [SLP.star_assoc]

lemma keep_in_left_trivial {p} (l : SLP (State p)) {r rr}
    (h : r = (⟦⟧ ⋆ rr)) : (l ⋆ r) = (l ⋆ rr) := by
  kan_cases h
  kan_rw [@SLP.true_star (State p) _ rr]

lemma keep_in_left_nontrivial {p} (l : SLP (State p)) {r rl rr}
    (h : r = (rl ⋆ rr)) : (l ⋆ r) = ((l ⋆ rl) ⋆ rr) := by
  kan_cases h
  kan_rw [@SLP.star_assoc (State p) _ l rl rr]

lemma keep_in_right_trivial {p} (l : SLP (State p)) {r rl}
    (h : r = (rl ⋆ ⟦⟧)) : (l ⋆ r) = (rl ⋆ l) := by
  kan_cases h
  kan_rw [@SLP.star_true (State p) _ rl]
  kan_rw [@SLP.star_comm (State p) _ l rl]

lemma keep_in_right_nontrivial {p} (l : SLP (State p)) {r rl rr}
    (h : r = (rl ⋆ rr)) : (l ⋆ r) = (rl ⋆ (l ⋆ rr)) := by
  kan_cases h
  kan_rw [@SLP.star_comm (State p) _ l (rl ⋆ rr)]
  kan_rw [@SLP.star_assoc (State p) _ rl rr l]
  kan_rw [@SLP.star_comm (State p) _ rr l]

lemma mk_left {p} (l : SLP (State p)) : l = (l ⋆ ⟦⟧) := by
  kan_rw [@SLP.star_true (State p) _ l]

lemma mk_right {p} (r : SLP (State p)) : r = (⟦⟧ ⋆ r) := by
  kan_rw [@SLP.true_star (State p) _ r]

lemma pull_exi_left {α p} {P : α → SLP (State p)} {Q : SLP (State p)} :
    ((∃∃ x, P x) ⋆ Q) = ∃∃ x, (P x ⋆ Q) := by
  kan_rw [<- SLP.star_exists]

lemma pull_exi_right {α p} {P : SLP (State p)} {Q : α → SLP (State p)} :
    (P ⋆ (∃∃ x, Q x)) = ∃∃ x, (P ⋆ Q x) := by
  kan_rw [@SLP.star_comm (State p) _ P (∃∃ x, Q x)]
  kan_rw [<- SLP.exists_star]
...
partial def linearize (term : SLTerm) : TacticM (SLTerm × Lean.Expr) := do
  ...
  | SLTerm.star _ l r =>
    ...
    let eq ← mkAppM ``Internal.congr_star_r
      #[l.expr, r.expr, r_norm.expr, r_norm_eq]
...
partial def insert (term : SLTerm) : TacticM (SLTerm × Lean.Expr) := do
  ...
  | SLTerm.star e l (SLTerm.star _ ll r) =>
    if needs_swap l ll then
      let swapEq ← mkAppM ``Internal.do_star_swap #[l.expr, ll.expr, r.expr]
      let swappedRExpr ← mkAppM ``SLP.star #[l.expr, r.expr]
      let (insertedR, insertedREq) ← insert (SLTerm.star swappedRExpr l r)
      let finalExpr ← mkAppM ``SLP.star #[ll.expr, insertedR.expr]
      let insertedEq ← mkAppM ``Internal.congr_star_r
        #[ll.expr, swappedRExpr, insertedR.expr, insertedREq]
...
partial def sort (term : SLTerm) : TacticM (SLTerm × Lean.Expr) := do
  ...
  | SLTerm.star _ l r =>
    ...
    let eq ← mkAppM ``Internal.congr_star_r
      #[l.expr, r.expr, r_sorted.expr, r_sorted_eq]
...
partial def norm (term : SLTerm) : TacticM (SLTerm × Lean.Expr) := do
  let simpResult ← simpOnlyNames
    [``SLP.exists_star, ``SLP.star_exists, ``SLP.star_true, ``SLP.true_star]
    term.expr
...
partial def surfaceExis (term : SLTerm) : TacticM (Lean.Expr × Lean.Expr) := do
  let simpResult ← simpOnlyNames
    [``Internal.pull_exi_left, ``Internal.pull_exi_right] term.expr
...
/-- Splits `term` into `L ⋆ R` according to `pred`, returning `L`, `R`, and a
proof that `term = L ⋆ R`.  Preserves the ordering of `L` and `R`; if either
side would be empty, `.unit` is produced. -/
partial def split_by (pred : SLTerm → TacticM SplitDirection)
    (term : SLTerm) : TacticM (SLTerm × SLTerm × Lean.Expr) := do
  match term with
  | SLTerm.star _ l r =>
    let (rl, rr, req) ← split_by pred r
    match ← pred l with
    | .left =>
      match rl with
      | .unit _ =>
  ...
  | other =>
    ...
    let unit ← mkAppOptM ``SLP.lift
      #[some alpha, some inst, some <| mkConst ``True []]
